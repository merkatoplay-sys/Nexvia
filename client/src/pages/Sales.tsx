...
// ✅ MISMA LÓGICA DE SLOTS QUE EN ACCOUNTS
const buildDisplayProfiles = (account: any) => {
...
export default function Sales() {
  ...
  // ✅ AHORA USA LA MISMA LÓGICA VISUAL DE SLOTS
  const selectedAccountProfiles = useMemo(() => {
    return selectedAccount ? buildDisplayProfiles(selectedAccount) : [];
  }, [selectedAccount]);
    ...
    <div className="space-y-6 sm:space-y-8">
                  ...
                  >
                    ...
                    <SelectContent className="bg-popover border-white/10 text-white">
                      {filteredAccounts.map((acc: any) => {
                        ...
                        const profiles = buildDisplayProfiles(acc);
      ...
      <div className="grid gap-3 sm:gap-6 grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
        {filteredAccounts.map((account: any) => {
          const displayProfiles = buildDisplayProfiles(account);
                          ...
                          <div
                            ...
                            onClick={() => {
                              ...
                              if (p.status === 'disponible') {
                                ...
                                const idToUse = p.__placeholder ? (realAvailable[0]?.id ?? p.id) : p.id;
                  ...
                  {!sold && availableCount > 0 && (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setSelectedServiceId(account.serviceId || selectedServiceId);
                        setSelectedAccountId(account.id);
                        setSelectedProfileId(realAvailable[0]?.id ?? displayProfiles.find((p: any) => p.status === 'disponible')?.id ?? '');
                        setIsSellDialogOpen(true);
                      }}
                    >
